...
//    Client credentials - used when the client itself is the resource owner (one client does not operate with multiple users), client credentials are exchanged directly for the tokens
	...
	public void configure(final ClientDetailsServiceConfigurer clients) throws Exception {
		 ...
		 * .secret("{noop}secret").autoApprove(true)
		 * .authorizedGrantTypes("refresh_token", "password", "authorization_code")
		 * .scopes("foo", "read", "write")
		 * //.redirectUris("http://193.1.4.52:8080/fintp_ui/login?fp=Login",
		 * "http://centos7toss-dezv:8080/fintp_ui/login?fp=Login",
		 * "http://localhost:8085/fintp_ui/login?fp=Login");
		 * .redirectUris("http://localhost:8085/fintp_ui/login?fp=Login");
		 */
